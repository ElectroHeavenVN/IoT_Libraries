...
#ifndef HTTPClientMod_H_
#define HTTPClientMod_H_
...
/// HTTP client errors
#define HTTPCLIENTMOD_HTTPC_ERROR_CONNECTION_REFUSED  (-1)
...
#define HTTPCLIENTMOD_TCP_RX_BUFFER_SIZE (4096)
#define HTTPCLIENTMOD_TCP_TX_BUFFER_SIZE (1460)
...
typedef struct {
  ...
  bool secure = false;
} HttpClientMod_Cookie;
typedef std::vector<HttpClientMod_Cookie> HttpClientMod_CookieJar;
 ...
 */
  ...
  /// Cookie jar support
  void setCookieJar(HttpClientMod_CookieJar *cookieJar);
  void resetCookieJar();
  void clearAllCookies();

// Async
  int sendRequestAsync(const char *type, String payload);
  int sendRequestAsync(const char *type, uint8_t *payload = NULL, size_t size = 0);
  ...
  bool asyncTimeout();
  String getStringAsync(void);
  int sendGetAsync();
  int sendDeleteAsync();
  int sendPostAsync(uint8_t *payload, size_t size);
  int sendPostAsync(String payload);
  int sendPutAsync(uint8_t *payload, size_t size);
  int sendPutAsync(String payload);
  int sendPatchAsync(uint8_t *payload, size_t size);
  int sendPatchAsync(String payload);
...
protected:
...
#ifdef HTTPCLIENT_1_1_COMPATIBLE
  HttpClientMod_TransportTraitsPtr _transportTraits;
  ...
  String _host;
  uint16_t _port = 0;
  int32_t _connectTimeout = HTTPCLIENTMOD_DEFAULT_TCP_TIMEOUT;
  bool _reuse = true;
  ...
  /// Cookie jar support
  HttpClientMod_CookieJar *_cookieJar = nullptr;
