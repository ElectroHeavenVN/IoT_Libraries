...
public:
  virtual ~HttpClientMod_TransportTraits() {}
...
class TLSTraits : public HttpClientMod_TransportTraits {
public:
 ...
 * @return success bool
 */
bool HTTPClientMod::begin(NetworkClient &client, String url) {
#ifdef HTTPCLIENT_1_1_COMPATIBLE
  if (_tcpDeprecated) {
 ...
 * @return success bool
 */
bool HTTPClientMod::begin(NetworkClient &client, String host, uint16_t port, String uri, bool https) {
#ifdef HTTPCLIENT_1_1_COMPATIBLE
  if (_tcpDeprecated) {
    log_d("mix up of new and deprecated api");
    _canReuse = false;
...
#ifdef HTTPCLIENT_1_1_COMPATIBLE
#ifndef HTTPCLIENT_NOSECURE
bool HTTPClientMod::begin(String url, const char *CAcert) {
  ...
  _secure = true;
  _transportTraits = HttpClientMod_TransportTraitsPtr(new TLSTraits(CAcert));
  if (!_transportTraits) {
 ...
 * @param url String
 */
bool HTTPClientMod::begin(String url) {
  if (_client && !_tcpDeprecated) {
    log_d("mix up of new and deprecated api");
    _canReuse = false;
    end();
  ...
  _transportTraits = HttpClientMod_TransportTraitsPtr(new HttpClientMod_TransportTraits());
  if (!_transportTraits) {
    log_e("could not create transport traits");
    return false;
  }
...
bool HTTPClientMod::beginInternal(String url, const char *expectedProtocol) {
...
#ifdef HTTPCLIENT_1_1_COMPATIBLE
bool HTTPClientMod::begin(String host, uint16_t port, String uri) {
  if (_client && !_tcpDeprecated) {
    log_d("mix up of new and deprecated api");
    _canReuse = false;
    end();
...
#ifndef HTTPCLIENT_NOSECURE
bool HTTPClientMod::begin(String host, uint16_t port, String uri, const char *CAcert) {
  if (_client && !_tcpDeprecated) {
    log_d("mix up of new and deprecated api");
    _canReuse = false;
    end();
...
bool HTTPClientMod::begin(String host, uint16_t port, String uri, const char *CAcert, const char *cli_cert, const char *cli_key) {
  if (_client && !_tcpDeprecated) {
    log_d("mix up of new and deprecated api");
    _canReuse = false;
    end();
  ...
  _secure = true;
  _transportTraits = HttpClientMod_TransportTraitsPtr(new TLSTraits(CAcert, cli_cert, cli_key));
  return true;
}
#endif  // HTTPCLIENT_NOSECURE
#endif  // HTTPCLIENT_1_1_COMPATIBLE
 ...
 */
void HTTPClientMod::disconnect(bool preserveClient) {
  if (connected()) {
 ...
 * @param reuse bool
 */
void HTTPClientMod::setReuse(bool reuse) {
  _reuse = reuse;
}
 ...
 */
void HTTPClientMod::setUserAgent(const String &userAgent) {
 ...
 */
void HTTPClientMod::setAcceptEncoding(const String &acceptEncoding) {
 ...
 */
void HTTPClientMod::setAuthorization(const char *user, const char *password) {
  if (user && password) {
    String auth = user;
    auth += ":";
    auth += password;
 ...
 */
void HTTPClientMod::setAuthorization(const char *auth) {
  if (auth) {
 ...
 */
void HTTPClientMod::setAuthorizationType(const char *authType) {
  if (authType) {
 ...
 */
void HTTPClientMod::setConnectTimeout(int32_t connectTimeout) {
 ...
 */
void HTTPClientMod::setTimeout(uint16_t timeout) {
  _tcpTimeout = timeout;
  if (connected()) {
 ...
 * @param use
 */
void HTTPClientMod::useHTTP10(bool useHTTP10) {
 ...
 * @return http code
 */
int HTTPClientMod::POST(uint8_t *payload, size_t size) {
  return sendRequest("POST", payload, size);
}

int HTTPClientMod::POST(String payload) {
 ...
 * @return http code
 */
int HTTPClientMod::PATCH(uint8_t *payload, size_t size) {
  return sendRequest("PATCH", payload, size);
}

int HTTPClientMod::PATCH(String payload) {
 ...
 * @return http code
 */
int HTTPClientMod::PUT(uint8_t *payload, size_t size) {
  return sendRequest("PUT", payload, size);
}

int HTTPClientMod::PUT(String payload) {
 ...
 * @return
 */
int HTTPClientMod::sendRequest(const char *type, String payload) {
 ...
 */
int HTTPClientMod::sendRequest(const char *type, uint8_t *payload, size_t size) {
  int code;
  bool redirect = false;
  ...
  do {
    ...
    // connect to server
    if (!connect()) {
      return returnError(HTTPCLIENTMOD_HTTPC_ERROR_CONNECTION_REFUSED);
    }
    ...
    // send Header
    if (!sendHeader(type)) {
      return returnError(HTTPCLIENTMOD_HTTPC_ERROR_SEND_HEADER_FAILED);
    }
    ...
    if (payload && size > 0) {
      ...
      if (sent_bytes != size) {
        return returnError(HTTPCLIENTMOD_HTTPC_ERROR_SEND_PAYLOAD_FAILED);
      }
    }
    ...
    //
    // Implementing HTTPCLIENTMOD_HTTP_CODE_FOUND as redirection with GET method,
    // to follow most of existing user agent implementations.
    //
    redirect = false;
    if (_followRedirects != HTTPCLIENTMOD_HTTPC_DISABLE_FOLLOW_REDIRECTS && redirectCount < _redirectLimit && _location.length() > 0) {
      switch (code) {
        // redirecting using the same method
        case HTTPCLIENTMOD_HTTP_CODE_MOVED_PERMANENTLY:
        case HTTPCLIENTMOD_HTTP_CODE_TEMPORARY_REDIRECT:
        {
          if (
              // allow to force redirections on other methods
              // (the RFC require user to accept the redirection)
              _followRedirects == HTTPCLIENTMOD_HTTPC_FORCE_FOLLOW_REDIRECTS ||
        ...
        // note: it does not need `HTTPCLIENTMOD_HTTPC_FORCE_FOLLOW_REDIRECTS` for any method
        case HTTPCLIENTMOD_HTTP_CODE_FOUND:
        case HTTPCLIENTMOD_HTTP_CODE_SEE_OTHER:
        {
          redirectCount += 1;
 ...
 * @return -1 if no info or > 0 when Content-Length is set by server
 */
int HTTPClientMod::sendRequest(const char *type, Stream *stream, size_t size) {

  if (!stream) {
    return returnError(HTTPCLIENTMOD_HTTPC_ERROR_NO_STREAM);
  }
  ...
  int buff_size = HTTPCLIENTMOD_TCP_TX_BUFFER_SIZE;
  ...
  // if possible create smaller buffer then HTTPCLIENTMOD_TCP_TX_BUFFER_SIZE
  ...
  if (buff) {
    ...
    return returnError(HTTPCLIENTMOD_HTTPC_ERROR_TOO_LESS_RAM);
  }
 ...
 * @return -1 if no info or > 0 when Content-Length is set by server
 */
int HTTPClientMod::getSize(void) {
  return _size;
}
 ...
 */
NetworkClient &HTTPClientMod::getStream(void) {
  if (connected()) {
    return *_client;
  }
 ...
 * @return NetworkClient*
 */
NetworkClient *HTTPClientMod::getStreamPtr(void) {
  if (connected()) {
    return _client;
  }
 ...
 */
int HTTPClientMod::writeToStream(Stream *stream) {
  ...
  if (!connected()) {
    return returnError(HTTPCLIENTMOD_HTTPC_ERROR_NOT_CONNECTED);
  }
  ...
  if (_transferEncoding == HTTPCLIENTMOD_HTTPC_TE_IDENTITY) {
  ...
  } else if (_transferEncoding == HTTPCLIENTMOD_HTTPC_TE_CHUNKED) {
    int size = 0;
    while (1) {
      if (!connected()) {
        return returnError(HTTPCLIENTMOD_HTTPC_ERROR_CONNECTION_LOST);
      }
      String chunkHeader = _client->readStringUntil('\n');

      if (chunkHeader.length() <= 0) {
        return returnError(HTTPCLIENTMOD_HTTPC_ERROR_READ_TIMEOUT);
      }
      ...
      if (len > 0) {
        ...
        if (ret != _size) {
          return returnError(HTTPCLIENTMOD_HTTPC_ERROR_STREAM_WRITE);
        }
        break;
      }
    ...
    return returnError(HTTPCLIENTMOD_HTTPC_ERROR_ENCODING);
  }
 ...
 * @return String
 */
String HTTPClientMod::getString(void) {
 ...
 * @return String
 */
String HTTPClientMod::errorToString(int error) {
  switch (error) {
    case HTTPCLIENTMOD_HTTPC_ERROR_CONNECTION_REFUSED:  return F("connection refused");
    case HTTPCLIENTMOD_HTTPC_ERROR_SEND_HEADER_FAILED:  return F("send header failed");
    case HTTPCLIENTMOD_HTTPC_ERROR_SEND_PAYLOAD_FAILED: return F("send payload failed");
    case HTTPCLIENTMOD_HTTPC_ERROR_NOT_CONNECTED:       return F("not connected");
    case HTTPCLIENTMOD_HTTPC_ERROR_CONNECTION_LOST:     return F("connection lost");
    case HTTPCLIENTMOD_HTTPC_ERROR_NO_STREAM:           return F("no stream");
    case HTTPCLIENTMOD_HTTPC_ERROR_NO_HTTP_SERVER:      return F("no HTTP server");
    case HTTPCLIENTMOD_HTTPC_ERROR_TOO_LESS_RAM:        return F("too less ram");
    case HTTPCLIENTMOD_HTTPC_ERROR_ENCODING:            return F("Transfer-Encoding not supported");
    case HTTPCLIENTMOD_HTTPC_ERROR_STREAM_WRITE:        return F("Stream write error");
    case HTTPCLIENTMOD_HTTPC_ERROR_READ_TIMEOUT:        return F("read Timeout");
 ...
 * @param first
 */
void HTTPClientMod::addHeader(const String &name, const String &value, bool first, bool replace) {
...
void HTTPClientMod::collectAllHeaders(bool collectAll) {
  _collectAllHeaders = collectAll;
}

void HTTPClientMod::collectHeaders(const char *headerKeys[], const size_t headerKeysCount) {
  if (_collectAllHeaders) {
...
String HTTPClientMod::header(const char *name) {
  for (size_t i = 0; i < _currentHeaders.size(); ++i) {
...
String HTTPClientMod::header(size_t i) {
...
String HTTPClientMod::headerName(size_t i) {
  if (i < _currentHeaders.size()) {
...
int HTTPClientMod::headers() {
  return _currentHeaders.size();
}

bool HTTPClientMod::hasHeader(const char *name) {
  for (size_t i = 0; i < _currentHeaders.size(); ++i) {
 ...
 */
bool HTTPClientMod::connect(void) {
  if (connected()) {
    if (_reuse) {
  ...
  if (!_client) {
    log_d("HTTPClientMod::begin was not called or returned error");
    return false;
  }
#ifdef HTTPCLIENT_1_1_COMPATIBLE
 ...
 * @return status
 */
bool HTTPClientMod::sendHeader(const char *type) {
  if (!connected()) {
    return false;
  }
 ...
 */
int HTTPClientMod::handleHeaderResponse() {

  if (!connected()) {
    return HTTPCLIENTMOD_HTTPC_ERROR_NOT_CONNECTED;
  }
  ...
  _transferEncoding = HTTPCLIENTMOD_HTTPC_TE_IDENTITY;
      ...
      if (headerLine == "") {
        ...
        if (transferEncoding.length() > 0) {
          ...
          if (transferEncoding.equalsIgnoreCase("chunked")) {
            _transferEncoding = HTTPCLIENTMOD_HTTPC_TE_CHUNKED;
          } else if (transferEncoding.equalsIgnoreCase("identity")) {
            _transferEncoding = HTTPCLIENTMOD_HTTPC_TE_IDENTITY;
          } else {
            return HTTPCLIENTMOD_HTTPC_ERROR_ENCODING;
          }
        } else {
          _transferEncoding = HTTPCLIENTMOD_HTTPC_TE_IDENTITY;
        }
        ...
        if (_returnCode) {
          ...
          return HTTPCLIENTMOD_HTTPC_ERROR_NO_HTTP_SERVER;
        }
      }

    } else {
      if ((millis() - lastDataTime) > _tcpTimeout) {
        return HTTPCLIENTMOD_HTTPC_ERROR_READ_TIMEOUT;
      }
      delay(10);
    }
  }

  return HTTPCLIENTMOD_HTTPC_ERROR_CONNECTION_LOST;
}
 ...
 */
int HTTPClientMod::writeToStreamDataBlock(Stream *stream, int size) {
  int buff_size = HTTPCLIENTMOD_TCP_RX_BUFFER_SIZE;
  int len = size;
  int bytesWritten = 0;

  // if possible create smaller buffer then HTTPCLIENTMOD_TCP_RX_BUFFER_SIZE
  if ((len > 0) && (len < buff_size)) {
    buff_size = len;
  }
  ...
  if (buff) {
      ...
      if (sizeAvailable) {
        ...
        if (bytesWrite != bytesRead) {
          ...
          if (bytesWrite != leftBytes) {
            ...
            free(buff);
            return HTTPCLIENTMOD_HTTPC_ERROR_STREAM_WRITE;
          }
        }
    ...
    return HTTPCLIENTMOD_HTTPC_ERROR_TOO_LESS_RAM;
  }
 ...
 * @return error
 */
int HTTPClientMod::returnError(int error) {
  if (error < 0) {
...
void HTTPClientMod::setFollowRedirects(httpclientmod_followRedirects_t follow) {
  _followRedirects = follow;
}

void HTTPClientMod::setRedirectLimit(uint16_t limit) {
 ...
 * @param url
 */
bool HTTPClientMod::setURL(const String &url) {
...
const String &HTTPClientMod::getLocation(void) {
  return _location;
}

void HTTPClientMod::setCookieJar(HttpClientMod_CookieJar *cookieJar) {
  _cookieJar = cookieJar;
}

void HTTPClientMod::resetCookieJar() {
  _cookieJar = nullptr;
}

void HTTPClientMod::clearAllCookies() {
  if (_cookieJar) {
    _cookieJar->clear();
  }
}

void HTTPClientMod::setCookie(String date, String headerValue) {
  if (!_cookieJar) {
    return;
  }
#define HTTP_TIME_PATTERN "%a, %d %b %Y %H:%M:%S"

  HttpClientMod_Cookie cookie;
  String value;
  int pos1, pos2;
...
bool HTTPClientMod::generateCookieString(String *cookieString) {
  time_t now_local = time(NULL);
  struct tm tm_gmt;
  gmtime_r(&now_local, &tm_gmt);
  time_t now_gmt = mktime(&tm_gmt);
...
int HTTPClientMod::sendRequestAsync(const char* type, String payload)
{
  return sendRequestAsync(type, (uint8_t *)payload.c_str(), payload.length());
}
 ...
 * @return -1 if no info or > 0 when Content-Length is set by server
 */
int HTTPClientMod::sendRequestAsync(const char *type, uint8_t *payload, size_t size) {
  int code;
  bool redirect = false;
  uint16_t redirectCount = 0;
  // wipe out any existing headers from previous request, but preserve the keys if collecting specific headers
  ...
  // connect to server
  if (!connect()) {
    return asyncHttpCode = returnError(HTTPCLIENTMOD_HTTPC_ERROR_CONNECTION_REFUSED);
  }
  ...
  // send Header
  if (!sendHeader(type)) {
    return asyncHttpCode = returnError(HTTPCLIENTMOD_HTTPC_ERROR_SEND_HEADER_FAILED);
  }
  ...
  if (payload && size > 0) {
    ...
    if (sent_bytes != size) {
      return asyncHttpCode = returnError(HTTPCLIENTMOD_HTTPC_ERROR_SEND_PAYLOAD_FAILED);
    }
  }

  lastSendRequestAsyncTime = millis();
  asyncHttpCode = 0;
  return asyncHttpCode;
}
 ...
 * @return -1 if no info or > 0 when Content-Length is set by server
 */
int HTTPClientMod::sendRequestAsync(const char *type, Stream *stream, size_t size) {

  if (!stream) {
    return asyncHttpCode = returnError(HTTPCLIENTMOD_HTTPC_ERROR_NO_STREAM);
  }
  ...
  if (buff) {
    ...
    log_d("too less ram! need %d", buff_size);
    return asyncHttpCode = returnError(HTTPCLIENTMOD_HTTPC_ERROR_TOO_LESS_RAM);
  }
 ...
 * asynchronously process the response from the server
 * @return int http code
 */
int HTTPClientMod::processResponseAsync() {

  if (!connected()) {
    return asyncHttpCode = HTTPCLIENTMOD_HTTPC_ERROR_NOT_CONNECTED;
  }
  if (asyncTimeout()) {
      return asyncHttpCode = returnError(HTTPCLIENTMOD_HTTPC_ERROR_READ_TIMEOUT);
  }    
  size_t len = _client->available();
  if(len <= 0) //Server did not send any data yet
    return 0;
  ...
  while (connected()) {
    ...
    if (len > 0) {
      ...
      if ((millis() - lastSendRequestAsyncTime) > _tcpTimeout) {
        return asyncHttpCode = HTTPCLIENTMOD_HTTPC_ERROR_READ_TIMEOUT;
      }
      delay(10);
    }
  }

  return asyncHttpCode = HTTPCLIENTMOD_HTTPC_ERROR_CONNECTION_LOST;
}

int HTTPClientMod::cancelRequestAsync()
{
    if(!connected()) {
        return asyncHttpCode = returnError(HTTPCLIENTMOD_HTTPC_ERROR_NOT_CONNECTED);
    }
    end();
    if (_client)
        _client->stop();
    return asyncHttpCode = returnError(HTTPCLIENTMOD_HTTPC_ASYNC_ERROR_CANCELLED);
}
 ...
 * @return String
 */
String HTTPClientMod::getStringAsync(void) {
  // _size can be -1 when Server sends no Content-Length header
  if (_size > 0 || _size == -1) {
    StreamString sstring;
    // try to reserve needed memory (noop if _size == -1)
    if (sstring.reserve((_size + 1))) {
      asyncHttpCode = writeToStream(&sstring);
      return sstring;
    } else {
      log_d("not enough memory to reserve a string! need: %d", (_size + 1));
    }
...
int HTTPClientMod::sendGetAsync()
{
    return sendRequestAsync("GET");
}

int HTTPClientMod::sendDeleteAsync()
{
    return sendRequestAsync("DELETE");
}

int HTTPClientMod::sendPostAsync(uint8_t* payload, size_t size)
{
    return sendRequestAsync("POST", payload, size);
}

int HTTPClientMod::sendPostAsync(String payload)
{
    return sendPostAsync((uint8_t *) payload.c_str(), payload.length());
}

int HTTPClientMod::sendPutAsync(uint8_t* payload, size_t size)
{
    return sendRequestAsync("PUT", payload, size);
}

int HTTPClientMod::sendPutAsync(String payload)
{
    return sendPutAsync((uint8_t *) payload.c_str(), payload.length());
}

int HTTPClientMod::sendPatchAsync(uint8_t* payload, size_t size)
{
    return sendRequestAsync("PATCH", payload, size);
}

int HTTPClientMod::sendPatchAsync(String payload)
{
    return sendPatchAsync((uint8_t *) payload.c_str(), payload.length());
}

int HTTPClientMod::available()
{
    return _client->available();
}

//Must read response text before calling this
int HTTPClientMod::asyncResponseCode()
{
    return asyncHttpCode;
}

bool HTTPClientMod::asyncTimeout()
{
    return (millis() - lastSendRequestAsyncTime) > _tcpTimeout;
}
